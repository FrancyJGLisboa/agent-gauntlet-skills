...
for(const entry of fs.existsSync(root)?fs.readdirSync(root,{withFileTypes:true}):[]){
  ...
  // Normalized because Git checks these files out with CRLF on Windows, where every
  // \n in the patterns below would otherwise stop matching and report valid frontmatter
  // as invalid.
